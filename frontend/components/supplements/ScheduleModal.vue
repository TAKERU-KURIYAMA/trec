  ...
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4">
      ...
      <div class="relative bg-white dark:bg-gray-800 rounded-lg max-w-md w-full p-6">
        <h2 class="text-xl font-bold mb-4">スケジュールを追加</h2>
        ...
        <form @submit.prevent="handleSubmit">
          <div class="space-y-4">
            ...
            <div>
              <label class="block text-sm font-medium mb-1">曜日設定 <span class="text-red-500">*</span></label>
              <select
                v-model="form.daysOfWeek"
                required
                class="w-full px-3 py-2 border border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600"
              >
                <option v-for="days in daysOfWeekOptions" :key="days.value" :value="days.value">
                  {{ days.label }}
                </option>
              </select>
            </div>
...
const { timingTypes, daysOfWeekOptions } = useSupplement()
